...
console.log("Hi");
	...
	init = function( event ) {
		...
		canvas.width = red.parentElement.clientWidth;
		canvas.height = document.body.scrollHeight-4;
		canvas.style.position = 'absolute'
		canvas.style.top = '0px'
		document.getElementById('red').appendChild( canvas );
    ...
    context.fillStyle = "rgba(255, 255, 255, 0.4)";
        ...
        points[i] = { x: (i/pointCount) * (canvas.width+2000), y: 100 };
 ...
 //       var noise = CanvasPS3.noise(p.o.x+tn, p.o.y+tn, tn);
        var noise = CanvasPS3.noise(p.x*0.002+tn, p.o.y*0.02+tn, tn);
        //var noise = CanvasPS3.noise(p.o.x, p.o.y, tn);
     		p.x = p.o.x + noise*50-10;
        p.y = (noise * 800);
      }
			...
			// Slowly erase
			context.fillStyle = "rgba(189, 212, 222, 0.04)";
			context.fillRect(0, 0, canvas.width, canvas.height);
			context.save();
      context.lineWidth = 25;
			...
			for(i = 0; i < points.length; ++i) {
				...
				context.strokeStyle = "rgba(" + hsv[1] + "," + hsv[1] +"," + hsv[2] + ", 1)";
				context.stroke();
				 	context.closePath();
